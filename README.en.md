...
├── plot_tb_results.py      # [NEW] TensorBoard data extraction & academic plotting script
...
### 1. 🎮 Frontend JRPG Web Arena & LLM Autoplay Demo
In the React web arena, you can play manually or turn on the **AI AUTO** / **EVOLUTION** modes to observe real-time decision-making and battle analysis of the Gemini models:
![Frontend JRPG Web Arena & LLM Autoplay Demo](assets/web_play_demo.gif)

### 2. 🤖 Reinforcement Learning (PPO) Agent Tactics Demo
The RL agent trained with Stable-Baselines3 (PPO) autonomously learns perfect defensive Taunt timings and low-HP self-destruction without any human-coded hard rules:
![RL Agent Gameplay Demo](assets/rl_agent_demo.gif)

### 3. 📈 TensorBoard Contrastive Training Curves (PPO Baseline vs. PPO Detailed)
Below is the scientific metrics comparison plot logged during the contrastive experiments of `train_baseline.py` (minimalist reward) vs. `train_rl.py` (expert-detailed reward), generated by `plot_tb_results.py`:
![TensorBoard Comparison Chart](assets/tensorboard_comparison.png)

> [!NOTE]
> **🎓 Scientific Insights & Hypothesis Verification**:
> - **Cumulative Damage Per Episode (DPE)**: In the early phase (first 200k steps), `PPO_Detailed` (expert mode) converges faster due to dense tactical incentives (e.g., healing rewards). However, it is prone to **Reward Hacking** (the agent intentionally drops health to trigger Ellie's high-reward sacrifice transfusion, leading to early wipeouts), causing the damage to stagnate around 2000. In contrast, `PPO_Baseline` (minimalist mode) avoids early reward traps and eventually achieves higher maximum limits closer to the theoretical maximum (Oracle: 5037.0).
> - **Survival Turns (ST)**: As multi-agent coordination matures, survival turns increase, demonstrating that the AI successfully learns Arthur's cycle-3 defensive Taunt (reducing 999 execute damage by 70% to 299) to mitigate the Boss's execution.
